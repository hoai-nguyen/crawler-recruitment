...
class TimViecNhanhCrawler extends Controller{
    ...
    public function TimViecNhanh($start_page, $step){
        ...
        while($x < $end) { //todo for in range
			...
			echo "i = ".$x.": ";
            $pageUrl = 'https://www.timviecnhanh.com/vieclam/timkiem?&page='.$x;
    		$crawler = $client -> request('GET', $pageUrl);
			...
			$jobs -> each(
		    	function ($node) {
					try {
                        ...
                        //todo separate
                        $fp = fopen('timviecnhanh-links.csv', 'a');
    ...
    public function TimJob($url){
		...
		if ($content_crawler -> count() <= 0 ) {
			...
			foreach ($quanti_info as $node) {
				...
				if (strpos($label, 'Mức lương') !== false){
					$salary = $quanti_crawler -> text();
					$reach += 1;
				} else if (strpos($label, 'Số lượng tuyển dụng') !== false){
				...
				if ($side_bar -> count() > 0){
					foreach ($side_bar as $node) {
						...
						if ($label -> count() > 0){
							...
							if (strpos($label_text, 'Website') !== false){
								$text = $node_crl -> text();
	...
	public function Test(){
    	...
    	$url = "https://www.timviecnhanh.com/tuyen-nhan-vien-theo-doi-don-hang-bo-phan-kinh-doanh-ho-chi-minh-4329844.html";
    	$crawler = $client->request('GET',$url);
		$job_title = $crawler -> filter('header.block-title > h1 > spanx') -> text();
