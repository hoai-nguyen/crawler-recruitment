...
if [[ ! " ${array[@]} " =~ "$job_name" ]] || [ "$job_name" = "" ]; then
    ...
    echo "Available job_name: topdev, topcv, itviec, mywork, timviecnhanh, vieclam24h, findjobs, careerlink, timviec365, laodong, tuyencongnhan, tuyendungcomvn, tuyendungsinhvien, itguru, tenshoku, tenshokuex, hatalike, rikunabi."	
    exit 1
fi
